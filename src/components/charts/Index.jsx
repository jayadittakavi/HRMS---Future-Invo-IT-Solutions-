        ...
        <div style={{ height, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '10px', paddingTop: '20px' }}>
            {data.map((item, index) => (
                <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '5px', textAlign: 'center' }}>{item.label}</span>
                </div>
            ))}
        </div>
    );
...
/**
 * Composite Chart (Bar + Line Overlay)
 * @param {Array} data - Array of objects { label, value, color, lineValue }
 * @param {string} height - Height of the chart container
 */
export const SimpleCompositeChart = ({ data = [], height = '250px' }) => {
    const maxBarValue = Math.max(...data.map(d => d.value || 0)) || 100;
    const maxLineValue = Math.max(...data.map(d => d.lineValue || 0)) || 100;

    // Normalize line points to SVG coordinates (0-100)
    const points = data.map((d, i) => {
        const x = (i / (data.length - 1)) * 100;
        const y = 100 - ((d.lineValue / maxLineValue) * 100);
        return `${x},${y}`;
    }).join(' ');

    return (
        <div style={{ position: 'relative', height, paddingTop: '20px' }}>
            {/* SVG Line Overlay */}
            <svg
                viewBox="0 0 100 100"
                ...
                style={{
                    position: 'absolute',
                    top: '20px',
                    left: '5%', // Align with bars roughly
                    width: '90%',
                    height: 'calc(100% - 40px)', // Adjust for labels
                    overflow: 'visible',
                    zIndex: 2,
                    pointerEvents: 'none'
                }}
            >
                <polyline
                    points={points}
                    fill="none"
                    stroke="#10b981" // Green trend line
                    strokeWidth="3"
                    strokeLinecap="round"
            ...
            {/* Bars */}
            <div style={{ height: '100%', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '5px' }}>
                {data.map((item, index) => (
                    <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, zIndex: 1 }}>
                        {/* Floating Label for Line (Mock) */}

                        <div
                            style={{
                                width: '60%',
                                height: `${(item.value / maxBarValue) * 80}%`, // Leave room for line at top
                                backgroundColor: item.color || '#3b82f6',
                                borderRadius: '4px 4px 0 0',
                                transition: 'height 0.5s ease',
                                position: 'relative'
                        ...
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', marginTop: '5px' }}>{item.label}</span>
                    </div>
                ))}
            </div>
        </div>
    ...
    return (
        <div style={{ position: 'relative', width: size, height: size, margin: '0 auto' }}>
            ...
            <div style={{
                width: '70%',
                height: '70%',
                backgroundColor: 'var(--glass-bg)', // Adaptive glass bg
