...
const Employees = () => {
    ...
    const [menuOpenId, setMenuOpenId] = useState(null); // three‑dot menu state
    ...
    const closeEditModal = () => setEditEmp(null);

    const { updateEmployeeInfo } = usePermissions();

    const handleSaveEdit = async () => {
        if (!editEmp) return;
        setSaving(true);
        // simulate async save (replace with real API later)
        setTimeout(() => {
            const updated = {
                ...editEmp,
                name: `${editEmp.firstName} ${editEmp.lastName}`.trim(),
                email: editEmp.personalEmail,
                phone: editEmp.phone,
                // avatar optional – keep previous if unchanged
                avatar: editEmp.avatar
            };
            // Update via context helper
            updateEmployeeInfo(editEmp.id, updated);
            closeEditModal();
            setSaving(false);
    ...
    const toggleStatus = (emp) => {
        ...
        if (window.confirm(`Are you sure you want to set this employee to ${newStatus}?`)) {
            updateEmployeeInfo(emp.id, { status: newStatus });
                                    ...
                                    <tr key={emp.id} className="employee-row-animate">
                                        ...
                                        <td className="text-center">
                                            <div className="d-flex justify-content-center align-items-center gap-2">
                                                <button className="btn-icon-action security" title="Edit Permissions" onClick={() => setSelectedEmpForPerms(emp)}>
                                                    <FiShield size={16} />
                                                </button>
                                                <button className="btn-icon-action" title={emp.status === 'Active' ? 'Deactivate' : 'Activate'} onClick={() => toggleStatus(emp)}>
                                                    {emp.status === 'Active' ? <FiCheckCircle size={16} /> : <FiXCircle size={16} />}
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
